...
<body class="dHome-body institu" data-offset="50" data-target=".subnav" data-spy="scroll">
			...
			<div class="form-horizontal">
				<div class="control-group">
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
     			      <c:choose>
						 	<c:when test="${periodical.endYear== 3000}">
								至今
							</c:when>
							<c:otherwise>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls padding">
	         			<a class="btn btn-primary" href="<dhome:url value="/institution/${domain}/backend/periodical/update/${periodical.id }?returnPage=${returnPage }"/>">编辑</a>
						<a id="deletePeriodical" class="btn" data-url="<dhome:url value="/institution/${domain }/backend/periodical/delete?perId[]=${periodical.id }"/>">删除</a>
	...
	<script>
		$(document).ready(function(){
			$('#deletePeriodical').on('click',function(){
				if(confirm("期刊任职删除以后不可恢复，确认删除吗？")){
					$.get($(this).data('url')).done(function(data){
						if(data.success){
							window.location.href='<dhome:url value="/institution/${domain}/backend/periodical/list/${returnPage }"/>'
						}else{
							alert(data.desc);
						}
					});
			...
			$('#perName').html(names['${periodical.name}']);
			
			//职位
			var positions = ["主要负责人","项目负责人","参与者"];
			$('#position').html(positions['${periodical.position}']);
			...
			//渲染作者
			$.get('<dhome:url value="/institution/${domain}/backend/periodical/authors/${periodical.id}"/>').done(function(data){
				if(data.success){
