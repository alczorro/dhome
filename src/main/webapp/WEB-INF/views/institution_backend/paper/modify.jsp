...
ul#paperTable li span.title {width:280px; font-weight:bold; color:#08a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
</style>
...
<body class="dHome-body institu" data-offset="50" data-target=".subnav" data-spy="scroll">
	...
	<div class="container">
		...
		<div class="ins_backend_rightContent">
			...
			<jsp:include page="../../commonPaperBackend.jsp"><jsp:param name="activeItem" value="modifyPaper" /> </jsp:include>
<!-- 				<li>&nbsp;&nbsp;&nbsp;</li> -->
<!-- 				<li> -->
<%-- 				    <a href="<dhome:url value="/institution/${domain }/backend/paper/list/1"/>">论文 </a> --%>
<!-- 				</li> -->
<!-- 				<li class="active"> -->
<!-- 				    <a href="#"> -->
<%-- 					    <c:choose> --%>
...
<%-- 	<script src="<dhome:url value="/resources/scripts/jquery/jquery-2.1.1.min.js"/>" type="text/javascript"></script> --%>
<%-- 	<script src="<dhome:url value="/resources/scripts/moment-with-locales.js"/>" type="text/javascript"></script> --%>
...
//   			$("#publicationYear").datepicker({ picker: "<img class='picker' align='middle' src='<dhome:url value='/resources/third-party/datepicker/sample-css/cal.gif'/>' alt=''>"});
...
// 					}
					...
					$.post('<dhome:url value="/institution/${domain}/backend/paper/submit"/>',data).done(function(data){ 
						if(data.success){
// 							window.location.href='<dhome:url value="/institution/${domain}/backend/paper/list/${empty returnPage?1:returnPage}"/>'
							if('${op}'=='update'){
...
// 					 	},
			...
			new qq.FileUploader({
				...
				action :'<dhome:url value="/institution/${domain}/backend/paper/upload"/>',
			...
			$("#title").on('blur',function(){
				if('${op}'!='update'){
				$("#paperTable").empty();
				$.post("<dhome:url value='/institution/${domain}/backend/paper/getPaperByTitle?title="+$("#title").val()+"'/>").done(function(data){
					if(data.success){
						window.name='<dhome:url value="/institution/${domain}/backend/paper/list/1"/>';
						for(var i=0;i<data.data.length;i++){
							
							if(data.data[i].doi==null){
								$("#paperTable").append('<li><span class="title" title="'+data.data[i].title+'">'+data.data[i].title+'</span><span class="doi">doi:空</span><span><a class="paper-edit" data-pid="'+data.data[i].id+'"><i class="icon icon-edit"></i> </a></span></li>');
							}else{
								$("#paperTable").append('<li><span class="title" title="'+data.data[i].title+'">'+data.data[i].title+'</span><span class="doi" title="'+data.data[i].doi+'">doi:'+data.data[i].doi+'</span><span><a class="paper-edit" data-pid="'+data.data[i].id+'"><i class="icon icon-edit"></i> </a></span></li>');
							}
							
						}
// 						$("#btn-update").show();
//  						$("#btn-update").attr("href","<dhome:url value='/institution/${domain}/backend/paper/update/"+data.data.id+"'/>");
					}
				});
				}
			});
			$('.paper-edit').live('click',function(){
					...
					window.location.href="<dhome:url value='/institution/${domain}/backend/paper/update/"+pid+"'/>";
			});
			...
			})(50,parseInt('${paper.publicationYear}'));
			//发表月份
			...
			if('${op}'=='update'){
				//渲染作者
				$.get('<dhome:url value="/institution/${domain}/backend/paper/authors/${paper.id}"/>').done(function(data){
					if(data.success){
