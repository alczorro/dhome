		...
		<div class="row-fluid mini-layout center p-top">
			...
			<div class="span9 left-b">
				<div class="ins_backend_rightContent">
					<jsp:include page="../../commonPeriodical.jsp"><jsp:param name="activeItem" value="modifyPeriodical" /> </jsp:include>
					<form id="editForm" class="form-horizontal">
						<input type="hidden" name="id" id="id" value="${empty per.id?0:per.id }"/>
						
			<%-- 			<div class="control-group">
			       			<label class="control-label"><span class="red">*</span>期刊名称：</label>
			       			<div class="controls">
			         			<input type="text" name="name" id="name" value="<c:out value="${per.name }"/>" class="register-xlarge"/>
			       		...
			       		<div class="control-group">
			       			...
			       			<div class="controls">
			         			<select id="name" name="name">
			         				<c:forEach items="${periodicalNames.entrySet() }" var="data">
			         			         <option value="${data.key }" <c:if test="${data.key==per.name }">selected="selected"</c:if>>${data.value.val}</option>
			         			 	</c:forEach>
								</select>
			       			</div>
			       		</div>
						...
						<div class="control-group">
			       			...
			       			<div class="controls">
			         			<select id="departId" name="departId">
			         				<option value="0" <c:if test="${per.departId==0 }">selected="selected"</c:if>>--</option>
			         				<c:forEach items="${deptMap.entrySet() }" var="data">
			         			         <option value="${data.key }" <c:if test="${data.key==per.departId }">selected="selected"</c:if>>${data.value.shortName}</option>
			         			 	</c:forEach>
								</select>
			       			</div>
			       		</div>
			       			...
			       			<div class="controls">
			         			<select id="position" name="position">
			         				<c:forEach items="${positions.entrySet() }" var="data">
			         			         <option value="${data.key }" <c:if test="${data.key==per.position }">selected="selected"</c:if>>${data.value.val}</option>
			         			 	</c:forEach>
								</select>
			       			</div>
			       		</div>
...
<!-- 								<p class="notFind">没有找到您要选择的任职人员？点击这里<a id="addAuthor">添加任职人员</a></p> -->
			       		...
			       		<div class="control-group">
			       			...
			       			<div class="controls">
			         			<input type="button" id="periodicalSaveBtn" value="提交" class="btn btn-primary"/>
			         			<a class="btn btn-link" href="<dhome:url value="/people/${domain }/admin/periodical/list/1"/>">取消</a>
			       			</div>
			       		</div>
	...
	<script>
		$(document).ready(function(){
			...
			$('.removeAuthor').live('click',function(){
				if(confirm("任职人员删除以后不可恢复，确认删除吗？")){
					var email=$(this).data('uid');
					author.remove(email);
				}
			});
			//查询用户
			$("#authorSearch").autocomplete('<dhome:url value="/people/${domain}/admin/academic/search/user"/>',
			            {
					  		width:400,
							parse:function(data){
			...
			$('#authorSaveBtn').on('click',function(){
				...
				$.post('<dhome:url value="/people/${domain}/admin/periodical/author/create"/>',$('#addAuthorForm').serialize()).done(function(data){
					if(data.success){
			...
			(function(loop,startYear,endYear){
				...
				$('#endYear').val(endYear);
			})(15,parseInt('${per.startYear}'),parseInt('${per.endYear}')); 
...
// 				}
			...
			})(12,parseInt('${per.startMonth}'),parseInt('${per.endMonth}')); 
			
		   //提交论文
			$('#periodicalSaveBtn').on('click',function(){
				if($('#editForm').valid()){
...
// 							data+='&authorTeacher[]='+auth.authorTeacher;
					...
					$.post('<dhome:url value="/people/${domain}/admin/periodical/submit"/>',data).done(function(data){ 
						if(data.success){
							window.location.href='<dhome:url value="/people/${domain}/admin/periodical/list/1"/>'
						}else{
							alert(data.desc);
						}
					});
			...
			if('${op}'=='update'){
				//渲染作者
				$.get('<dhome:url value="/people/${domain}/admin/periodical/authors/${per.id}"/>').done(function(data){
					if(data.success){
