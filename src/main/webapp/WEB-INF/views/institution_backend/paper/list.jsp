		...
		<div class="container">
			...
			<div class="ins_backend_rightContent">
				<ul class="nav nav-tabs">
					<jsp:include page="../../commonPaperBackend.jsp"><jsp:param name="activeItem" value="papers" /> </jsp:include>
				    <li class="search">
				...
				<ul class="listShow">
					...
					<c:forEach items="${page.datas }" var="data">
						<li>
							...
							<span class="article">
								<a class="title" href="<dhome:url value="/institution/${domain }/backend/paper/detail/${data.id }?returnPage=${page.currentPage }"/>"><c:out value="${data.title }"/></a>
...
// 				papersId=${papersId};
				var url='/institution/${domain }/backend/paper/importAll';
				search(url);
// 				for(var i=0;i<papersId.length;i++){
// 					url+="&papersId[]="+papersId[i];
// 				}
// 				url+="&papersId[]="+papersId;
// 			  	window.location.href="<dhome:url value='/institution/${domain }/backend/paper/importAll?page=${page.currentPage}'/>"+url;
			});
			$('#batchBtn').on('click',function(){
				...
				if(operation=='batchDelete'){
					...
					if(confirm("论文删除以后不可恢复，确认删除吗？")){
						$.get("<dhome:url value='/institution/${domain}/backend/paper/delete?page=${page.currentPage}'/>"+url).done(function(data){
							if(data.success){
								window.location.reload();
							}else{
								alert(data.desc);
					...
					window.location.href="<dhome:url value='/institution/${domain}/backend/paper/batchImport?page=${page.currentPage}'/>"+url;
// 					$.get("<dhome:url value='/institution/${domain}/backend/paper/batchImport?page=${page.currentPage}'/>"+url).done(function(data){
// 						if(data.success){
// 							window.location.reload();
// 						}else{
// 							alert(data.desc);
...
// 					});
		...
		/* 	$('#citeRefresh').on('click',function(){
				...
				$.get("<dhome:url value='/institution/${domain}/backend/paper/cite/'/>"+paperId).done(function(data){
					if(data.success){
