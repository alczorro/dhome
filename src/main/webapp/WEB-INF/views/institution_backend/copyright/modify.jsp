...
<body class="dHome-body institu" data-offset="50" data-target=".subnav" data-spy="scroll">
	...
	<div class="container">
		...
		<div class="ins_backend_rightContent">
			...
			<jsp:include page="../../commonCopyrightBackend.jsp"><jsp:param name="activeItem" value="modifyCopyright" /> </jsp:include>
<!-- 				<li>&nbsp;&nbsp;&nbsp;</li> -->
<!-- 				<li> -->
<%-- 				    <a href="<dhome:url value="/institution/${domain }/backend/copyright/list/1"/>">软件著作权${copyrightCount }</a> --%>
<!-- 				</li> -->
<!-- 				<li class="active"> -->
<!-- 				    <a href="#"> -->
<%-- 					    <c:choose> --%>
...
<!-- 			    </li> -->
			...
			<form id="editForm" class="form-horizontal" method="post" action="<dhome:url value="/institution/backend/copyright/creat"/>">
				<input type="hidden" name="id" id="id" value="${empty copyright.id?0:copyright.id }"/>
				
				<div class="control-group">
	       			<label class="control-label"><span class="red">*</span>成果名称：</label>
	       			<div class="controls">
	         			<input type="text" name="name" id="name" class="register-xlarge" value="<c:out value="${copyright.name }"/>" maxlength="250"/>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			<input type="text" name="registerNo" id="registerNo" class="register-xlarge" value="<c:out value="${copyright.registerNo }"/>" maxlength="250"/>
						<span id="registerNo_error_place" class="error"></span>
	       			</div>
	       		</div>
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			<select id="departId" name="departId">
	         				<option value="0" <c:if test="${copyright.departId==0 }">selected="selected"</c:if>>--</option>
	         				<c:forEach items="${deptMap.entrySet() }" var="data">
	         			         <option value="${data.key }" <c:if test="${data.key==copyright.departId }">selected="selected"</c:if>>${data.value.shortName}</option>
	         			 	</c:forEach>
						</select>
	       			</div>
	       		</div>
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			<select id="type" name="type">
	         				<option value="0" <c:if test="${copyright.type==0 }">selected="selected"</c:if>>--</option>
	         				<c:forEach items="${types.entrySet() }" var="data">
	         			        <option value="${data.key }" <c:if test="${data.key==copyright.type }">selected="selected"</c:if>>${data.value.val}</option>
	         			 	</c:forEach>
						</select>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			<select id="grade" name="grade">
	         				<option value="0" <c:if test="${copyright.grade==0 }">selected="selected"</c:if>>--</option>
	         				<c:forEach items="${grades.entrySet() }" var="data">
	         			         <option value="${data.key }" <c:if test="${data.key==copyright.grade }">selected="selected"</c:if>>${data.value.val}</option>
	         			 	</c:forEach>
						</select>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
						...
						<p class="hint">请输入软件设计人的姓名、邮箱或者单位，按回车确认添加。</p>
						<p class="notFind">没有找到您要选择的软件设计人？点击这里<a id="addAuthor">添加软件设计人</a></p>
	       			</div>
...
<%-- 						<a id="fileRemove" style="display:${empty paper||paper.clbId==0?'none':'inline' }">删除</a> --%>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			<input type="button" id="copyrightSaveBtn" value="提交" class="btn btn-primary"/>
	         			<a class="btn btn-link" href="<dhome:url value="/institution/${domain }/backend/copyright/list/1"/>">取消</a>
	       			</div>
	       		</div>
			</form>
		</div>	
		...
		<div class="modal-header">
           ...
           <h3>添加软件设计人</h3>
			...
			<div class="modal-body">
				<div class="control-group">
         			<label class="control-label">软件设计人排序：</label>
          			<div class="controls">
          				第
...
// 				$data.authorTeacher=$('#editAuthorTeacherL input[type=checkbox]').is(':checked');
			...
			$('#authorSaveBtn').on('click',function(){
				...
				$.post('<dhome:url value="/institution/${domain}/backend/copyright/author/create"/>',$('#addAuthorForm').serialize()).done(function(data){
					if(data.success){
...
// 					 	},
			...
			//提交课题
			$('#copyrightSaveBtn').on('click',function(){
				if($('#editForm').valid()){
...
// 							data+='&authorType[]='+auth.authorType;
					...
					$.ajax({
	 					  type: 'POST',
	 					  url: '<dhome:url value="/institution/${domain}/backend/copyright/submit"/>',
	 					  data: data,
	 					  success: function(data){window.location.href='<dhome:url value="/institution/${domain}/backend/copyright/list/${empty returnPage?1:returnPage}"/>'},
...
// 					});
			...
			new qq.FileUploader({
				...
				action :'<dhome:url value="/institution/${domain}/backend/copyright/upload"/>',
...
// 			});
			...
			if('${op}'=='update'){
				//渲染软件设计人
				$.get('<dhome:url value="/institution/${domain}/backend/copyright/authors/${copyright.id}"/>').done(function(data){
					if(data.success){
				...
				$.get('<dhome:url value="/institution/${domain}/backend/copyright/attachments/${copyright.id}"/>').done(function(data){
					if(data.success){
		...
		})(6,parseInt('${copyright.companyOrder}'));
	</script>
	<!-- 软件设计人表格模板 -->
