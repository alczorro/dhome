 ...
 */
@Controller
@RequestMapping("/people/{domain}/paper.dhome")
public class ShowPaperController {
	...
	public ModelAndView display(HttpServletRequest request, @PathVariable("domain") String domain) {
		...
		if(null != targetUser){
			//判断该用户是否做的迁移
			int isMove=userService.getSimpleUser(targetUser.getId()).getIsMove();
			int flag=0;
			if(isMove==1){
				flag=2;
			}else{
			...
			JSONArray papers = PaperRender.format(paperService.getPapers(targetUser.getId(), 0, 0), dsnService.getUserNameString(targetUser.getId()));
			...
			mv.addObject("IAPPaper",backendPaperService.getPapersByUID(targetUser.getId()));
