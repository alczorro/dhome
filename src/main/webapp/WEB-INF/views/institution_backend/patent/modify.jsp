	...
	<div class="container">
		...
		<div class="ins_backend_rightContent">
			<ul class="nav nav-tabs">
				<jsp:include page="../../commonPatentBackend.jsp"><jsp:param name="activeItem" value="modifyPatent" /> </jsp:include>
			</ul>
			<form id="editForm" class="form-horizontal" method="post" action="<dhome:url value="/institution/backend/patent/creat"/>">
				...
				<div class="control-group">
	       			...
	       			<div class="controls">
	         			<input type="text" name="name" id="name" class="register-xlarge" value="<c:out value="${patent.name }"/>" maxlength="250"/>
...
<%-- 						<a id="fileRemove" style="display:${empty paper||paper.clbId==0?'none':'inline' }">删除</a> --%>
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			...
	         			<a class="btn btn-link" href="<dhome:url value="/institution/${domain }/backend/patent/list/1"/>">取消</a>
	       			</div>
	       		</div>
			</form>
		</div>	
...
// 				$data.authorTeacher=$('#editAuthorTeacherL input[type=checkbox]').is(':checked');
			...
			$('#authorSaveBtn').on('click',function(){
				...
				$.post('<dhome:url value="/institution/${domain}/backend/patent/author/create"/>',$('#addAuthorForm').serialize()).done(function(data){
					if(data.success){
...
// 							data+='&authorType[]='+auth.authorType;
					...
					$.ajax({
	 					  type: 'POST',
	 					  url: '<dhome:url value="/institution/${domain}/backend/patent/submit"/>',
	 					  data: data,
	 					  success: function(data){window.location.href='<dhome:url value="/institution/${domain}/backend/patent/list/${empty returnPage?1:returnPage}"/>'},
...
// 					});
			...
			new qq.FileUploader({
				...
				action :'<dhome:url value="/institution/${domain}/backend/patent/upload"/>',
