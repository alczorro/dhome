...
public class PermissionIntercepter extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        String targetDomain = extractDomain(requestURL);
        request.setAttribute("targetDomain", targetDomain);
        if (isSystemAdmin(requestURL)) {//判断是否进入后台管理
            return systemRedirect(request, response);
        }
        if (isAdminURL(requestURL)) {
            String currentDomain = (String) request.getSession().getAttribute(Constants.CURRENT_USER_DOMAIN);
            request.setAttribute("isAdminStatus", true);
            ...
            if (!isMySelf(targetDomain, currentDomain)) {
                gotoErrorPage(
                        response,
                        UrlUtil.getRootURL(request)
                                +"/people/" + currentDomain + "/index.html");
                return false;
            }
        }
        ...
        String methodName = getMethodNameFromHandler(handler);
        NPermission permission = findPermission(handler, methodName);
        if (permission == null) {
            return true;
        }
        if (!isAccessable(request, permission)) {
            return executeAccessDeniedProcess(request, response, handler, methodName, permission);
        }
        return true;
    }
        ...
        if (denyMethod != null) {
    		...
    		if(user!=null){
    			if(user.getInstitutionId()!=null){
        			...
        			gotoErrorPage(response, UrlUtil.getRootURL(request) + "/institution/" + home.getDomain() + "/permission");
    			}else{
    				gotoErrorPage(response, UrlUtil.getRootURL(request) + "/system/login?state=");
    			}
