	...
	<div class="container canedit">
		<div class="row-fluid mini-layout center p-top">
			...
			<div class="span9 left-b">
					...
					<jsp:include page="../../commonPeriodical.jsp"><jsp:param name="activeItem" value="periodicals" /> </jsp:include>
						<ul class="listShow">
							<li class="title">
							...
							<c:forEach items="${page.datas }" var="data">
								<li>
									<span class="pername">
									    ...
									    <span class="manage">
									    		<a class="label label-success" href="<dhome:url value="/people/${domain }/admin/periodical/detail/${data.id }?returnPage=${page.currentPage }"/>">查看</a>
												<a class="label label-success" href="<dhome:url value="/people/${domain }/admin/periodical/update/${data.id }?returnPage=${page.currentPage }"/>">编辑</a>
												<a class="label label-danger deletePeriodical" href="<dhome:url value="/people/${domain }/admin/periodical/delete?perId[]=${data.id }&page=${page.currentPage }"/>">删除</a>
										</span>
									</span>
									<span class="position">
										<c:choose>
		...
		$(document).ready(function(){
			...
			$('.authorDetailPopover').on('click',function(){
				...
				if($('#popover_'+authorId).size()==0){
					...
					$.get('<dhome:url value="/people/${domain}/admin/periodical/author/"/>'+perId+"/"+authorId).done(function(result){
						if(result.success){
							$self.popover({
