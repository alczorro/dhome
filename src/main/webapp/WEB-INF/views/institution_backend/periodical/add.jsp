...
<jsp:include page="../leftMenu.jsp"></jsp:include>
		<div class="ins_backend_rightContent">
			<ul class="nav nav-tabs">
				<jsp:include page="../../commonPeriodicalBackend.jsp"><jsp:param name="activeItem" value="modifyPeriodical" /> </jsp:include>
			</ul>
...
<!-- 						<p class="notFind">没有找到您要选择的任职人员？点击这里<a id="addAuthor">添加任职人员</a></p> -->
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls">
	         			...
	         			<a class="btn btn-link" href="<dhome:url value="/institution/${domain }/backend/periodical/list/1"/>">取消</a>
	       			</div>
	       		</div>
	...
	<script>
		$(document).ready(function(){
			...
			$('#authorSaveBtn').on('click',function(){
				...
				$.post('<dhome:url value="/institution/${domain}/backend/periodical/author/create"/>',$('#addAuthorForm').serialize()).done(function(data){
					if(data.success){
...
// 							data+='&authorTeacher[]='+auth.authorTeacher;
					...
					$.post('<dhome:url value="/institution/${domain}/backend/periodical/submit"/>',data).done(function(data){ 
						if(data.success){
							window.location.href='<dhome:url value="/institution/${domain}/backend/periodical/list/1"/>'
						}else{
							alert(data.desc);
						}
					});
			...
			if('${op}'=='update'){
				//渲染作者
				$.get('<dhome:url value="/institution/${domain}/backend/periodical/authors/${per.id}"/>').done(function(data){
					if(data.success){
