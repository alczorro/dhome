		...
		<div class="row-fluid mini-layout center p-top">
			...
			<div class="span9 left-b">
				<div class="ins_backend_rightContent">
					<jsp:include page="../../commonAcademic.jsp"><jsp:param name="activeItem" value="modifyAcademic" /> </jsp:include>
...
<!-- 								<p class="notFind">没有找到您要选择的任职人员？点击这里<a id="addAuthor">添加任职人员</a></p> -->
			       		...
			       		<div class="control-group">
			       			...
			       			<div class="controls">
			         			...
			         			<a class="btn btn-link" href="<dhome:url value="/people/${domain }/admin/academic/list/1"/>">取消</a>
			       			</div>
			       		</div>
...
// 						});
			...
			$('#authorSaveBtn').on('click',function(){
				...
				$.post('<dhome:url value="/people/${domain}/admin/academic/author/create"/>',$('#addAuthorForm').serialize()).done(function(data){
					if(data.success){
...
// 							data+='&authorTeacher[]='+auth.authorTeacher;
					...
					$.post('<dhome:url value="/people/${domain}/admin/academic/submit"/>',data).done(function(data){ 
						if(data.success){
							window.location.href='<dhome:url value="/people/${domain}/admin/academic/list/1"/>'
						}else{
							alert(data.desc);
						}
					});
			...
			if('${op}'=='update'){
				//渲染作者
				$.get('<dhome:url value="/people/${domain}/admin/academic/authors/${academic.id}"/>').done(function(data){
					if(data.success){
