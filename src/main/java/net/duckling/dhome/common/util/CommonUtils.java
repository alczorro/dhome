...
public final class CommonUtils {
	...
	public static <T>  List<Integer> extractSthField(List<T> list,String fieldName){
		...
		for(T t:list){
			...
			int objV=obj==null?0:Integer.parseInt(obj.toString());
			if(result.contains(objV)){
				continue;
			}
			result.add(objV);
		}
	...
	public static <T> Map<Integer,T> extractSthFieldToMap(List<T> list,String fieldName){
		if(CommonUtils.isNull(list)){
			return null;
		}
		...
		for(T t:list){
			Object obj=ReflectUtils.getValue(t, fieldName);
			result.put(obj==null?0:Integer.parseInt(obj.toString()),t);
		}
		return result;
	}
}
