...
<!-- 					<input type="button" id="batchBtn" value="确认" class="btn btn-primary"/> -->
				...
				<ul class="listShow">
					<li class="title">
						...
						<span class="pro-title">在职状态</span>
						<span class="status">操作</span>
					</li>
					<c:forEach items="${page.datas }" var="data">
						<li>
							<span class="check"><input data-uid="${data.uid }" type="checkbox" class="checkUser"/></span>
							<span class="employee">
...
<!-- 									</span> -->
							...
							<span class="status">
								<c:if test="${ !empty data.cstnetId }">
								<a href="<dhome:url value='/institution/${domain}/backend/achievements/report?achievementsId=${achievementsId}&umtId=${data.umtId }&uid=${data.uid }'/>" target="_blank" title="批次号：${data}" class="label label-success">导出绩效考核表</a>
							</c:if>
							</span>
						</li> 
					</c:forEach>
		...
		$(document).ready(function(){
			...
			$('#batchBtn').on('click',function(){
				...
				if(operation=='batchImport'){
					...
					alert(url);
					window.location.href="<dhome:url value='/institution/${domain}/backend/achievements/report?achievementsId=${achievementsId}'/>"+url;
				}else{
					alert('请选择批量操作'); 
				}
			});
