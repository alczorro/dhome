	...
	<body class="dHome-body institu" data-offset="50" data-target=".subnav" data-spy="scroll">
		...
		<div class="container">
			...
			<ul class="nav nav-tabs">
				<jsp:include page="../../commonPeriodicalBackend.jsp"><jsp:param name="activeItem" value="periodicals" /> </jsp:include>
				 <li class="search">
			    	<form class="bs-docs-example form-search">
			            <input id='periodicalKeyword' type="text" placeholder="请输入成果名称" class="input-medium search-query" value="<c:out value="${condition.keyword }"/>"/>
			            <button id="searchPeriodicalBtn" type="button" class="btn">搜索</button>
			        </form>
			    </li>
			</ul>
						...
						<li>
							...
							<span class="pername">
						    	<c:choose>
									...
									<c:otherwise>
										${periodicalNames[data.name].val}
									</c:otherwise>
								</c:choose>
							
							    <span class="manage">
							    		<a class="label label-success" href="<dhome:url value="/institution/${domain }/backend/periodical/detail/${data.id }?returnPage=${page.currentPage }"/>">查看</a>
										<a class="label label-success" href="<dhome:url value="/institution/${domain }/backend/periodical/update/${data.id }?returnPage=${page.currentPage }"/>">编辑</a>
										<a class="label label-danger deletePeriodical" data-url="<dhome:url value="/institution/${domain }/backend/periodical/delete?perId[]=${data.id }"/>">删除</a>
								</span>
							</span>
							<span class="position">
								<c:choose>
	...
	<script>
		$(document).ready(function(){
			...
			function search(baseUrl){
				...
				baseUrl+="&keyword="+encodeURIComponent($.trim($('#periodicalKeyword').val()));
			...
			$('#searchPeriodicalBtn').on('click',function(){
				search(1);
			});
			
			$('#periodicalKeyword').enter(function(){
				search(1);
			});
			...
			$('.authorDetailPopover').on('click',function(){
				...
				if($('#popover_'+authorId).size()==0){
					...
					$.get('<dhome:url value="/institution/${domain}/backend/periodical/author/"/>'+perId+"/"+authorId).done(function(result){
						if(result.success){
							$self.popover({
