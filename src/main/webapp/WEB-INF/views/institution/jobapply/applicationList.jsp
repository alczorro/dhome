...
<body class="dHome-body gray" data-offset="50" data-target=".subnav" data-spy="scroll">
			...
			<div class="span9 left-b">
				<div class="ins_backend_rightContent">
					...
					<c:forEach items="${page.datas }" var="data">
				    	...
				    	<c:if test="${data.deadline.time-nowDate>=0 }">
							<li>
	...
	<!-- 				    				</a> -->
					    		...
					    		<span class="position">
					    			<c:if test="${empty jobMap[data.id] }">
										<a id="popup-add" class="btn btn-mini btn-primary" data-toggle="modal" href="/people/${domain}/admin/job/details/${data.id }">申请</a>
									</c:if>
									<c:if test="${!empty jobMap[data.id] }">
										<a class="label label-info popup-edit" data-toggle="modal" href="/people/${domain}/admin/job/update/${jobMap[data.id].id }"><i class="icon-edit icon-white"></i>更新</a>
									</c:if>
								</span>
							</li>
						</c:if> 
