			...
			<div class="form-horizontal">
				<div class="control-group">
	       		...
	       		<div class="control-group">
	       			...
	       			<div class="controls padding">
	         			<a class="btn btn-primary" href="<dhome:url value="/institution/${domain}/backend/treatise/update/${treatise.id }?returnPage=${returnPage }"/>">编辑</a>
						<a id="deleteTreatise" class="btn" data-url="<dhome:url value="/institution/${domain }/backend/treatise/delete?treatiseId[]=${treatise.id }"/>">删除</a>
			...
			$('#deleteTreatise').on('click',function(){
				if(confirm("论著删除以后不可恢复，确认删除吗？")){
					$.get($(this).data('url')).done(function(data){
						if(data.success){
							window.location.href='<dhome:url value="/institution/${domain}/backend/treatise/list/${returnPage }"/>'
						}else{
							alert(data.desc);
						}
					});
