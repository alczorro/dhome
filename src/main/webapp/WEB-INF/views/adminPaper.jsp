		    ...
		    <div class="span9 left-b">
		    	<div id="mainSpan">
				    <div class="page-header">
					    <h3><fmt:message key='common.paper.title'/>
					    <span class="publish">							   		
					    	<c:if test="${status=='hide' }">
							(<fmt:message key='common.publish.notdo'/> <a class="btn btn-mini" href="<dhome:url value="/people/${domain}/admin/paper/changeStatus"/>"><fmt:message key='common.publish.do'/></a>)
							</c:if>
							<c:if test="${status=='show' }">
							(<fmt:message key='common.publish.done'/> <a class="btn btn-mini" href="<dhome:url value="/people/${domain}/admin/paper/changeStatus"/>"><fmt:message key='common.publish.undo'/></a>)
							</c:if>
						</span>
...
<%-- 							<input type="radio" name="paperLib" value="2" <c:if test="${flag==2 }">checked="checked"</c:if>/> IAP专用论文库 --%>
						...
						<c:if test="${isMove!=1 }">
					    	<a id="editPapers" href="<dhome:url value='/people/${domain}/admin/commonPaper'/>" class="float-right"><i class="icon-edit abs-top"></i><span class="common-link">编辑</span></a>
					   </c:if>
					   <c:if test="${isMove==1 }">
					    	<a id="editPapers" href="<dhome:url value='/people/${domain}/admin/paper/list/1'/>" class="float-right"><i class="icon-edit abs-top"></i><span class="common-link">编辑</span></a>
					   </c:if>
					   </h3>
					</div>
...
<%-- 									</c:if> --%>
							<!-- 发表时间 年.月 -->
							<c:if test="${data.publicationYear!=0 }">出版年:${data.publicationYear}</c:if><c:if test="${data.publicationMonth!=0 }">.${data.publicationMonth}</c:if>. 
						</span>
							<!-- 原文链接 -->
