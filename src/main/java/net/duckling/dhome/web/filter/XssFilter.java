 ...
 *
 */
package net.duckling.dhome.web.filter;
...
import net.duckling.dhome.common.util.HTMLFilter;
...
public class XssFilter implements Filter {
	...
	private class XssHttpServletRequestWrapper extends
		...
		public String getParameter(String name) {
			...
			if (isIgnoreparameter(name)) {
				...
				return HTMLFilter.makeSafe(param.trim());
			}
		...
		public String[] getParameterValues(String name) {
			...
			if (isIgnoreparameter(name)) {
				...
				int index = 0;
				for (String pa : param) {
					param[index++] = HTMLFilter.makeSafe(pa);
				}
				return param;
			}
	...
	private boolean isIgnoreUrl(String name) {
		if (name == null) {
			return false;
		}
		for (String ignoreUrl : ignoreUrlArray) {
			if (name.contains(ignoreUrl)) {
				return true;
			}
		}
		return false;
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		if (ignoreUrl != null && !ignoreUrl.isEmpty()) {
			ignoreUrlArray = ignoreUrl.split(splitChar);
		}
	}
}
